...
module.exports = class Expression {
  constructor(node, lambdaModule) {
    ...
    this.readyParsed = this.writeTree(this.parsed);
  ...
  getNext() {
    let future = reduceTree(this.next);
    if (future === null) { return null; }

    this.next = future;
    return this.writeTree(this.next);
  }
  ...
  getNormal() {
    ...
    if (!normal) {
      ...
      this.readyNormal = null;
      return;
    }

    this.normal = normal;
    return this.writeTree(normal);
  }
